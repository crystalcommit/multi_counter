...
>
  ...
  />
  <span class="text-lg font-bold px-4 absolute centerize">{counter.count}</span>
  <div class="ml-auto">
    <button
      on:click={addCount}
      class="text-white bg-red-500 rounded-l text-lg py-1 px-3 my-1">+</button
    >
    <button
    ...
    <button
      ...
      >0</button
    >
    <button on:click={handleRemove} class="text-gray-500 cursor-pointer px-4"
      >x</button
    >
  </div>
</div>
